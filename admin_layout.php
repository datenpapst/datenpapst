...
$pdo = get_db();
$type = ($_GET['type'] ?? 'admin') === 'tenant' ? 'tenant' : 'admin';
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $order = explode(',', $_POST['order'] ?? '');
    $table = $type === 'tenant' ? 'dashboard_widgets' : 'admin_widgets';
    $stmt = $pdo->prepare("UPDATE {$table} SET position=? WHERE widget=?");
    $pos = 1;
    foreach ($order as $w) {
        if ($w !== '') { $stmt->execute([$pos++, $w]); }
    }
    header('Location: admin_layout.php?type=' . $type);
    exit();
}
$table = $type === 'tenant' ? 'dashboard_widgets' : 'admin_widgets';
$widgets = $pdo->query("SELECT widget FROM {$table} ORDER BY position")->fetchAll(PDO::FETCH_COLUMN);
$title_key = $type === 'tenant' ? 'dashboard_layout' : 'admin_layout';
?>
<!DOCTYPE html>
...
<title><?php echo htmlspecialchars($site_title, ENT_QUOTES, 'UTF-8'); ?> - <?php echo t($title_key); ?></title>
...
<p><a href="admin.php"><?php echo t('admin_menu'); ?></a></p>
</body>
</html>
